...
@app.route('/users/stop-following/<int:follow_id>', methods=['POST'], endpoint='stop_following')
@login_required(context='user_details')
...
@app.route('/messages/<int:message_id>/delete', methods=
["POST"], endpoint='messages_destroy')
@login_required(context='user_details')
...
def toggle_like(message_id):
    ...
    check_for_like = Likes.query.filter_by(user_id=g.user.id, message_id = message_id).first()
...
def homepage():
    ...
    if g.user:
        messages = (Message
                    .query
                    .filter(Message.user_id.in_([following.id for following in g.user.following])| Message.id.in_([message.id for message in g.user.messages]))
                    .order_by(Message.timestamp.desc())
                    .limit(100)
                    .all())
